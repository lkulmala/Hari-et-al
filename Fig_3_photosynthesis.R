setwd('input/')
datafile<-read.table("Fig_3_photosynthesis.txt", header=TRUE)
...
C_meas<-datafile$C_p_meas  ## measured photosynthesis on day C  (mg CO2 m-2 s-1)
C_pred<-datafile$C_p_pred  ## predicted photosynthesis on day C (mg CO2 m-2 s-1)
...
mtext(expression('Photosynthesis (mg CO '[2]*' m'^-2*'day'^-1*')'),side=2, line=2, cex=0.85)

plot(y=C_meas, x=time, col="blue", ylim=c(-5,90), type="l",xlab="",ylab="")
lines(y=C_pred, x=time, col="black")
